/********************************
  ...
  * Program demonstrates simple data decomposition
  *
  * Source found at https://computing.llnl.gov/tutorials/mpi/samples/C/mpi_array.c
  ******************************/

#include "mpi.h"
#include <stdio.h>
#include <stdlib.h>
#define ARRAYSIZE 160000000
#define MASTER 0
...
int main (int argc, char*argv[])
{
    ...
    if (taskid > MASTER) {
        ...
        /* Receive mu portion of array from the master task */
        source = MASTER;
        MPI_Recv(&offset, 1, MPI_INT, source, tag1, MPI_COMM_WORLD, &status);
        MPI_Recv(&data[offset], chunksize, MPI_FLOAT, source, tag2, MPI_COMM_WORLD, &status);
